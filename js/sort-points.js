...
const filterRules = {
  ...
  'housing-features': (data, filter) => {
    // let featuresChecked = Array.from(filter.querySelectorAll('input[type="checkbox"]:checked')); // выбранные поля всё время меняются поэтому let
    let featuresChecked = []; // выбранные поля всё время меняются поэтому let
    featuresChecked = filter.querySelectorAll('input[type="checkbox"]:checked');
